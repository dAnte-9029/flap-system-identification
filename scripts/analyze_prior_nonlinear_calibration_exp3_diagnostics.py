#!/usr/bin/env python3
"""Diagnostics for Experiment 3 exact nonlinear prior calibration."""
...
from scripts.run_nested_prior_nonlinear_calibration_exp3 import DEFAULT_OUTPUT_ROOT
...
def _combined_fold_report(
    *,
    ...
    metrics = pd.read_csv(experiment_root / "experiment3_outer_test_metrics.csv")
    prior = metrics.loc[
        metrics["model"].str.endswith("_prior_only") & metrics["target"].isin(TARGETS),
        ["prior_name", "outer_fold", "target", "rmse", "r2"],
    ].rename(columns={"rmse": "prior_only_rmse", "r2": "prior_only_r2"})
    final = metrics.loc[
        metrics["model"].str.endswith("_gain_bias") & metrics["target"].isin(TARGETS),
        ["prior_name", "outer_fold", "target", "rmse", "r2"],
    ...
    report = report.merge(
        parameters.loc[:, parameter_columns],
...
def run(
    *,
    ...
    selected_alpha_path = experiment_root / "experiment3_selected_alpha_by_fold.csv"
    calibrated_path = experiment_root / "experiment3_calibrated_parameters_by_fold.csv"
    ...
    for _, row in calibrated.sort_values(["prior_name", "outer_fold"]).iterrows():
        ...
        prior_root = Path(str(row["exact_prior_root"]))
    ...
    phase_bins_all.to_csv(output_root / "experiment3_phase_bins_by_fold.csv", index=False)
    phase_all.to_csv(output_root / "experiment3_phase_summary_by_fold.csv", index=False)
    frequency_all.to_csv(output_root / "experiment3_frequency_energy_by_fold.csv", index=False)
    flap_main.to_csv(output_root / "experiment3_flap_fundamental_energy_by_fold.csv", index=False)
    decomposition_all.to_csv(output_root / "experiment3_gain_bias_decomposition_by_fold.csv", index=False)
    reconstruction_errors.to_csv(output_root / "experiment3_decomposition_reconstruction_check.csv", index=False)
    phase_summary.to_csv(output_root / "experiment3_phase_summary_mean_std.csv", index=False)
    flap_summary.to_csv(output_root / "experiment3_flap_fundamental_energy_mean_std.csv", index=False)
    decomposition_summary.to_csv(output_root / "experiment3_gain_bias_decomposition_mean_std.csv", index=False)
    fold_report.to_csv(output_root / "experiment3_fold_report.csv", index=False)

    comparison_parts: list[pd.DataFrame] = []
    for path in (
        PROJECT_ROOT / "artifacts/20260703_prior_shaping_ablation_exp1/diagnostics/experiment1_fold_report.csv",
        PROJECT_ROOT
        / "artifacts/20260703_prior_regression_calibration_independent_min_relaxed_probe/diagnostics/experiment2_fold_report.csv",
    ):
        if path.exists():
            comparison_parts.append(pd.read_csv(path))
    comparison_parts.append(fold_report)
    pd.concat(comparison_parts, ignore_index=True, sort=False).to_csv(
        output_root / "experiment3_comparison_with_exp1_exp2_fold_report.csv",
        index=False,
    )
    ...
    manifest = {
        ...
        "diagnostics": {
            "phase_residual_peak_to_peak": str(output_root / "experiment3_phase_summary_by_fold.csv"),
            "flap_fundamental_energy": str(output_root / "experiment3_flap_fundamental_energy_by_fold.csv"),
            "gain_bias_decomposition": str(output_root / "experiment3_gain_bias_decomposition_by_fold.csv"),
            "combined_fold_report": str(output_root / "experiment3_fold_report.csv"),
            "comparison_fold_report": str(output_root / "experiment3_comparison_with_exp1_exp2_fold_report.csv"),
        },
        "summary_outputs": {
            "phase": str(output_root / "experiment3_phase_summary_mean_std.csv"),
            "flap_fundamental": str(output_root / "experiment3_flap_fundamental_energy_mean_std.csv"),
            "gain_bias_decomposition": str(output_root / "experiment3_gain_bias_decomposition_mean_std.csv"),
        },
