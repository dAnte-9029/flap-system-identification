...
run ID：`20260717T054923Z_5b72620`

Git：`5b726205654fd263a1d59cd2cc230d73f592f1d5`

## 1. 执行摘要

1. **Fx 能量结构：**cycle-mean 占 per-log macro residual energy 的 10.7%，zero-mean wingbeat 占 89.3%；对应 `cycle_mean_residuals.csv` 与 Figure 3–4。
2. **Fz 能量结构：**cycle-mean 占 22.0%，zero-mean wingbeat 占 78.0%；对应 `cycle_mean_residuals.csv` 与 Figure 3–5。
3. **Fx phase：**fixed phase offset=-0.686 rad；fixed-delay estimate=-24.420 ms；H1/H2 circular RMSE=0.087/0.086 rad。
4. **Fz phase：**fixed phase offset=-0.243 rad；fixed-delay estimate=-8.726 ms；H1/H2 circular RMSE=0.128/0.124 rad。
...
7. **Phase correction：**Fx=`yes`，Fz=`yes`；建议 harmonic order range=[2, 4]；fx_b K1–K4 cumulative coverage=41.3%/93.1%/98.2%/98.7%；fz_b K1–K4 cumulative coverage=92.8%/95.7%/98.4%/99.2%。
8. **Condition dependence：**phase-conditioned 相对 phase-only 的 validation equal-log macro gain 为 Fx=16.2%、Fz=9.7%；建议变量=['angle_of_attack', 'flapping_frequency']。
9. **Short history：**最佳预定义短历史相对 static gain 为 Fx=9.6%、Fz=9.3%；dynamic model=`yes`，TCN=`no`。
10. **Label robustness：**label variant 的 phase-bin 平均 1-SD 为 0.840 N；decision=no；Fx optimal shift 对已有 variant 的变化=0.135 rad；Fz residual waveform variant correlation=0.991；对应 `label_robustness.csv`、`label_uncertainty_phase.csv` 与 Figure 13。
11. **DeLaurier prior value：**matched-capacity gain 为 Fx=-16.7%、Fz=-81.9%；incremental value=`no`。
12. **Trim：**WT1 sensitivity artifact 缺失，不能定量给出 trim frequency shift；仅报告 cycle-mean Fz discrepancy=4.185 N。
...
- Prior：`/home/zn/flap-system-identification/artifacts/20260717_delaurier_attitude_aware_3b5d4ec_trainval_v1` 的 wing-only DeLaurier force；artifact ID=`delaurier_attitude_aware_3b5d4ec_trainval_v1`，lifecycle=`active`，physics commit=`3b5d4ec1d28f1384cf042402992ad7ea59995f49`。
...
- cycle 总数 15700，接受 6553，拒绝比例 58.26%。
- 主要 rejection reasons：`{'phase_unwrap_not_monotonic': 8985, 'too_few_samples;incomplete_phase_coverage': 68, 'incomplete_phase_coverage': 38}`；没有静默丢弃 cycle。
...
Fx 的 per-log macro cycle-mean residual 为 -1.235 N，mean/WB energy fraction 为 10.7%/89.3%。固定 offset 与 fixed delay 的相对拟合见 Figure 2；单凭 shift 后 RMSE 下降不足以修改输入。绝对积分最大的 physical half-stroke 为 downstroke，equal-log macro 积分为 -4.512 N rad，跨 log 同号比例=100.0%。Joint condition probe gain 为 16.2%；单变量 U/AoA/f gain=1.4%/3.8%/10.1%。Cross-log 证据见 Figure 11–12，label-variant band 见 Figure 13。

## 5. Fz 误差归因

Fz 的 per-log macro cycle-mean residual 为 4.166 N，mean/WB energy fraction 为 22.0%/78.0%。绝对积分最大的 physical half-stroke 为 downstroke，equal-log macro 积分为 19.091 N rad，跨 log 同号比例=100.0%。该 half-stroke 指标来自 zero-mean waveform，因此半拍正负面积不会被误算为 cycle mean；4.166 N 的 cycle-mean discrepancy 是独立 branch，而不是由 zero-mean 半拍峰值代数产生。该 half-stroke identity 来自 canonical `q=A sin(phi)` direction helper，不使用未经验证的 `[0,pi)` 命名。Joint condition gain=9.7%；单变量 U/AoA/f gain=2.7%/5.5%/1.8%。WT1 sensitivity artifact 缺失，不能定量给出 trim frequency shift；仅报告 cycle-mean Fz discrepancy=4.185 N。 过大的半拍究竟是 localized peak、reversal 邻域还是整拍偏差，量化见 `half_stroke_residuals.csv` 与 Figure 5。
...
| label reconstruction | 部分支持 | label variant 的 phase-bin 平均 1-SD 为 0.840 N；decision=no |
| unsteady/flexible-wing memory | 部分支持 | linear short-history probe，不能推出 TCN 必要性 |

Component diagnostic：
- fx_b: 与 residual waveform shape correlation 绝对值最大的是 `dD_f` (equal-log mean r=-0.701)；这是候选关联，不是唯一物理因果。
- fz_b: 与 residual waveform shape correlation 绝对值最大的是 `dN_c` (equal-log mean r=-0.759)；这是候选关联，不是唯一物理因果。
...
- fx_b: 最大 absolute macro sensitivity similarity 为 `phase_offset_rad` (r=-0.823)；仍不是参数校准证据。
- fz_b: 最大 absolute macro sensitivity similarity 为 `normal_force_scale` (r=-0.943)；仍不是参数校准证据。

进入 decision matrix 的 physical parameter candidates：`['normal_force_scale']`（由可配置 similarity 与 step-stability thresholds 生成）。

## 7. Residual 可重复性

- fx_b: per-log vs macro r=0.992；same-date different-log r=0.987；cross-date r=0.982。
- fz_b: per-log vs macro r=0.990；same-date different-log r=0.985；cross-date r=0.975。
...
Matched-capacity comparison 使用相同 harmonic order、condition features、ridge grid、train/validation logs 与 sample weighting。Overall validation equal-log macro gain 为 Fx=-16.7%、Fz=-81.9%；cycle-mean RMSE gain 为 Fx=-38.6%、Fz=-190.4%。因此结构化 overall verdict=`no`：Fx/Fz overall 与 cycle mean 均未显示稳定增量价值；complete-log validation 之外没有独立 condition-extrapolation 证据。见 `matched_capacity_prior_probe.csv` 与 Figure 15。论文中宜描述为“在本 flight envelope 与 matched low-capacity basis 下，DeLaurier prior 未提供稳定 overall incremental information”，而不是预设 prior 必然有效。
...
- Output directory：`/home/zn/flap-system-identification/outputs/force_discrepancy_attribution/20260717T054923Z_5b72620`
- Git commit：`5b726205654fd263a1d59cd2cc230d73f592f1d5`
- Dataset：`/home/zn/flap-system-identification/dataset/canonical_v0.2_training_ready_split_measured_massprops_ratio8_sg0p03_v1`
- Split identity：`canonical_v0.2_training_ready_split_measured_massprops_ratio8_sg0p03_v1`
- Prior：`/home/zn/flap-system-identification/artifacts/20260717_delaurier_attitude_aware_3b5d4ec_trainval_v1`（`delaurier_attitude_aware_3b5d4ec_trainval_v1`，`active`）
...
```bash
python scripts/audit_force_discrepancy_attribution.py --dataset-root dataset/canonical_v0.2_training_ready_split_measured_massprops_ratio8_sg0p03_v1 --split-manifest dataset/canonical_v0.2_training_ready_split_measured_massprops_ratio8_sg0p03_v1/dataset_manifest.json --partitions train validation --output-root outputs/force_discrepancy_attribution
```
